...
let pipewidth = 64; 
let pipeheight = 512;
let pipeX = boardwidth; // Pipes spawn at the right edge of board
let pipeY = 0;          // Base Y coordinate
...
function update() {
    ...
    for (let i = pipesArray.length - 1; i >= 0; i--) {
        ...
        if (!pipe.passed && bird.x > pipe.x + pipe.width) {
            score += 0.5;// 0.5 become there two pipes! so 0.5*2 = 1 for each set of pipes
            pipe.passed = true;
        }
