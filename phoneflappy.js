...
let board;
let boardwidth = 360;     // Mobile phone width
let boardheight = 640;    // Mobile phone height
let context;

// Bird properties: width, height, initial position
let birdwidth = 36;      // Larger bird for mobile
let birdheight = 26;
...
let pipewidth = 70;    // Wider pipes for mobile
let pipeheight = 450;  // Slightly shorter pipes for playability
...
let openingSpace = 110;  // Wider gap for mobile play
...
function update() {
    ...
    for (let i = pipesArray.length - 1; i >= 0; i--) {
        ...
        if (!pipe.passed && bird.x > pipe.x + pipe.width) {
            score += 0.5; // 0.5 because each pipe set has two pipes
            pipe.passed = true;
        }
    ...
    // Extra cleanup: remove pipes from start if completely off-screen left
...
// Handle touch for mobile screens
...
// Collision detection between rectangles a and b (bird and pipe)
